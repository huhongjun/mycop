...
//--------------------------------------;
WebFXCheckBoxTreeItem.prototype.setChecked = function () {
	for(var i = 0; i < this.childNodes.length; i++){
		var divEl = document.getElementById(this.childNodes[i].id);
		
		var inputEl = divEl.getElementsByTagName("INPUT")[0];
		inputEl.checked=this.getChecked();

		this.doChildLadder(this.childNodes[i]);
	}

	if(this.getChecked()==true) this.doParentLadderTrue(this.parentNode);
	if(this.getChecked()==false) this.doParentLadderFalse(this.parentNode);
};

WebFXCheckBoxTreeItem.prototype.doChildLadder = function (crrNode) {
	if(crrNode==null) crrNode=this;
	if(crrNode.childNodes.length > 0){
		for(var i=0; i < crrNode.childNodes.length; i++){
			var divEl=document.getElementById(crrNode.childNodes[i].id);
			var inputEl=divEl.getElementsByTagName("INPUT")[0];
			inputEl.checked=crrNode.getChecked();
			this.doChildLadder(crrNode.childNodes[i]);
		}
	}
};

WebFXCheckBoxTreeItem.prototype.doParentLadderTrue = function (crrNode) {
	if(crrNode==null) return;
	var divEl=document.getElementById(crrNode.id);
	var inputEl=divEl.getElementsByTagName("INPUT")[0];
	inputEl.checked=true;
	this.doParentLadderTrue(crrNode.parentNode);
};

WebFXCheckBoxTreeItem.prototype.doParentLadderFalse = function (crrNode) {
	var childFlag=false;
	for(var i=0; i < crrNode.childNodes.length; i++){
		var divEl=document.getElementById(crrNode.childNodes[i].id);
		var inputEl=divEl.getElementsByTagName("INPUT")[0];
		if(inputEl.checked==true) childFlag=true;
	}
	if(childFlag == false){
		var divEl=document.getElementById(crrNode.id);
		var inputEl=divEl.getElementsByTagName("INPUT")[0];
		inputEl.checked=false;
		this.doParentLadderFalse(crrNode.parentNode);
	}
};