...
function WebFXLoadTree(sText, sXmlSrc,sAction,sScript,sBehavior, sIcon, sOpenIcon,sNodeid) {
	// call super
	this.WebFXTree = WebFXTree;
	this.WebFXTree(sText, sAction,sScript,sBehavior, sIcon, sOpenIcon,sNodeid);
...
WebFXLoadTree.prototype.reload  = function () {
     if (this.loaded) {
		var open = this.open;
		// remove
...
function WebFXLoadTreeItem(sText, sXmlSrc, sAction,sScript, eParent, sIcon, sOpenIcon,sNodeid) {
	// call super
	this.WebFXTreeItem = WebFXTreeItem;
	this.WebFXTreeItem(sText, sAction, sScript,eParent, sIcon, sOpenIcon,sNodeid);
...
function _xmlTreeToJsTree(oNode) {
    ...
    var isFolder=oNode.getAttribute("isFolder");
	var Script=oNode.getAttribute("Script");
	// create jsNode
	var jsNode;
    jsNode = new WebFXLoadTreeItem(text,src, action,Script, parent, icon, openIcon,nodeid);
	//保存对象 ，以便以后调用
	if(isFolder==null||isFolder=="false"){
        jsNode.isFolder=false;
	}else{
		jsNode.isFolder=true;
	}
	addNodetoArray(nodeid,jsNode);
	if (target != "")
...
////保存对象 ，以便以后调用的方法
var jnodes=new Array();
function addNodetoArray(nodeid,jsNode){
  var size=jnodes.length;
  for(i=0;i<size;i++){
    if(jnodes[i].id==nodeid){
       jnodes[i].node=jsNode;
	    return;
	}
  }
  var obj = new Object();
