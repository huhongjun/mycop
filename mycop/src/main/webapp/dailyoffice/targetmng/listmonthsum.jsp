...
function doSearch(){//校验页面--通过后提交
    ...
    doQuery('<%=context%>/dailyoffice/targetmng/listmonthsum.do',true)
	return true;
}
//-->
</SCRIPT>
<html>
<body class="bodybg">
  <html:form action="/dailyoffice/targetmng/listmonthsum" method="post">
  <table width="95%" border="0" align="center" cellpadding="1" cellspacing="1">
  <tr> 
    <td colspan="6">&nbsp;</td>
  </tr>
  <tr> 
    <td colspan="8" align="center" class="TableTitleText"><bean:write  name="MonthSumForm" property="deptnodename" filter="true"/>月度总结</td>
  </tr>
  <tr> 
    ...
    <td class="f12" colspan="8" align="right">
	 <table border="0" cellspacing="1" cellpadding="2">
		...
		<span class="InputLabelCell">月</span>
	      ...
	      <td>
	  <goa:button operation="YDZJ.VIEW" styleClass="button"  property="query" onclick="doSearch()" value="查询"/>
          </td> 
          <td><goa:button operation="YDZJ.OPT" styleClass="button"  property="add" onclick="doAdd()" value="新增"/></td>          
          <td>
          <% String temp="toUrl('"+context+"/dailyoffice/targetmng/editmonthsum.do?actionType=modify&flag=modify',true)";%>
	  <goa:button operation="YDZJ.OPT" styleClass="button"  property="modify" onclick="<%=temp%>" value="修改"/>
          </td> 
		  <!--
          <td>
          <% String temp1="toUrl('"+context+"/dailyoffice/targetmng/viewmonthsum.do',true)";%>
          <goa:button operation="YDZJ.VIEW"  styleClass="button" property="view" value="查看"  onclick="<%=temp1%>"/>
		  </td>
		  -->
          <td> 
		  <goa:button operation="YDZJ.OPT"  styleClass="button" property="delete" onclick="doDelete()"  value="删除"/>
          </td>		  
        </tr>
      </table>
	  </td>
  ...
  <tr> 
    ...
    <logic:iterate id="ps" name="MonthSumForm" property="pageControl.data" type="com.gever.goa.dailyoffice.targetmng.vo.TargetVO" indexId="index">
	<tr  onmouseover="javascript:changeBgColorOnMouseOver(this);" onmouseout="javascript:changeBgColorOnMouseOut(this);" ondblclick="<%="toUrl('"+context+"/dailyoffice/targetmng/viewmonthsum.do?fid=" +ps.getSerial_num()+"',false)"%>">
		...
		<% String link_url = ""+context+"/dailyoffice/targetmng/viewmonthsum.do?fid="+ ps.getSerial_num();%>
  ...
  <tr> 
    <td colspan="8" align="center" class="f12">
	    <html:hidden name="MonthSumForm" property="orderType"/>
        <html:hidden name="MonthSumForm" property="orderFld"/>
		<goa:pagelink name="MonthSumForm" property="pageControl" /> 
		</td>
	  </tr>
	</table>
</html:form>
