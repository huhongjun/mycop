...
<%@ page import="com.gever.sysman.organization.dao.impl.DepOrderDAOIMP"%>
<%@ page import="com.gever.sysman.organization.model.Department,java.util.*"%>
...
document.body.onmousedown=function(){
 ...
 <%
   DepOrderDAOIMP opDAO = new DepOrderDAOIMP();
   ...
   listop=opDAO.getDepartmentId(id);
   Iterator it = listop.iterator();
   //  while(it.hasNext()) {
	//	   Department M = (Department)it.next();
...
// alert(selectid);
 	function doBack(){
	document.location.href="<gdp:context/>/organization/DepartmentAction.do?action=list&departid="+resid;
	}
	...
	function addOptionMoveUp()
	{
	  ...
	  {    var prevOption = sourceSelect.options[sel_id-1];
		...
		//  moduleForm.action= action;
		 // moduleForm.submit();
 document.location.href="<gdp:context/>/organization/depOrderAction.do?action=moveDeparmentOrderId&id1="+prevOption.value+"&id2="+curOption.value+"&resid="+resid;
	...
	function addOptionMoveDown()
	{
	  ...
	  if (sourceSelect.length > 1 && sel_id < sourceSelect.length -1 && sel_id > -1) //got to have at least 2 items and not the last one is selected
	  {
		 ...
		 //      nextOption.value
			//   curOption.value
			//	   resid
document.location.href="<gdp:context/>/organization/depOrderAction.do?action=moveDeparmentOrderId&id1="+nextOption.value+"&id2="+curOption.value+"&resid="+resid;
...
<form action="/organization/depOrderAction" onsubmit="return false">
<p align="right"><br>
...
<tr>
  ...
  <tr>
    <td colspan="6" align="center" class="TableTitleText">部门排序</td>
  </tr>
<tr>
<td>
<table align="center">
          ...
          <tr>
            ...
            <select id="addRightListBox" name="addRightListBox"  multiple class="t12" style="width:120px; height:200px;" >
             <%
   while(it.hasNext()) {
   Department p = (Department)it.next();
    %>
