...
<%
/*处理思路:
 * 1.首先将后台的所有菜单数据存储到客户端javascript。
 * 2.将下拉菜单（二级以下含二级）的数据，装载到菜单数据当中，其中菜单数据是一个xml对象
 * 3.由于装载菜单时间有点长，故在主页的地方有做一个进度条（进度条的依据是根据，所有的菜单数据装载完毕后，才将主页显示出来。
 * 4.里面的处理可能有点复杂。关键是菜单的数据装载与进度条的处理。还有切换一级菜单
 */
...
//如果为NS,则增加HTMLElement的insertAdjacentElement,insertAdjacentHTML,insertAdjacentText方法,
//以缩小和IE的区别
if(ns){
...
<STYLE>
<!--
.picleft{background-repeat: no-repeat;
          ...
          <td  width="320" align="right"> <table width="220" height="33" border="0" cellpadding="0" cellspacing="0">
              <tr align="center" valign="top"> 
		       ...
		       %>
			    ...
			    <td>
				<%
		       String href4="/Login.do?action=logout";
		       %>
...
<iframe id="frmLog" src=""  frameBorder=0 style="display:none" ></iframe>
<SCRIPT LANGUAGE="JavaScript">
<!--
    //*****************************************************
	//资治国添加,解决netscape兼容性问题
	...
	if(menuCount ==0){
		document.all("loadtitle").innerText="正在载入桌面...";
		 <logic:notEqual name="UserMenuForm" property="isAdmin" value="true">
		//***********************************************************************************************************
		 //********************************************	
		 //资治国添加,解决netscape兼容性问题
		 //以前代码为:
	    ...
	    if(ie){
		 ...
		 if(ns){
		   //window.open("<%=context%>/jsp/homepage/right.jsp", 'frmcontent');
		   document.getElementById("frmcontent").contentDocument.location.href="<%=context%>/jsp/homepage/right.jsp";
		 }
        //**********************************************************************************************************
		 </logic:notEqual>
		<logic:equal name="UserMenuForm" property="isAdmin" value="true">
		//***********************************************************************************************************
		 //********************************************	
		 //资治国添加,解决netscape兼容性问题
		 //以前代码为:
		 //				   document.frames[0].document.location.href="<%=context%>/zzgl/usermng/usermngindex.do";
		 //********************************************
		 if(ie){
		   document.frames[0].document.location.href="<%=context%>/zzgl/usermng/usermngindex.do";
		 }
		 if(ns){
		   //window.open("<%=context%>/zzgl/usermng/usermngindex.do", 'frmcontent');
		   document.getElementById("frmcontent").contentDocument.location.href="<%=context%>/zzgl/usermng/usermngindex.do";
		 }
        //**********************************************************************************************************
		 nodeid2Path("060101");	
		 </logic:equal>
		...
		if(!initComplete){
			...
			if(menuCount--<=1){
				...
				<logic:equal name="UserMenuForm" property="isAdmin" value="true">
					...
					//document.all.mainContent.style="";
				initComplete=true;
...
//-->
//window.open("/gdp/debug.htm","debug", "height=150,width=400,resizable");
</SCRIPT>
</body>
</html>
