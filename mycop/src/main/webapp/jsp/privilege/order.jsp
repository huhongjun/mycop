...
<table width="95%" align="center">
  ...
  <tr> 
    <td colspan="2" class="TableTitleText">顺序调整</td>
  </tr>
  <tr> 
    <td colspan="2" class="">
...
<iframe width=200 height=300 id="sourceTree" src="<gdp:context/>/privilege/resourceAction.do?action=getTreeRoot&src=resourceAction&bNoView=1">
</iframe>
</td>
...
<br>	  		到资源:
    ...
    <td width="254">
	<iframe width=200 height=300 id="targetTree" src="<gdp:context/>/privilege/resourceAction.do?action=getTreeRoot&src=resourceAction&bNoView=1">
</iframe>
</td>
   ...
   //非法
	   ...
	   if(sourceParentNodeId==targetParentNodeId){
		  if(sourceNodeId==targetNodeId){
			...
			enableBtn(changePIDBtnEnabled,changePIDBtnDisabled,true);//2004-11-24
		   ...
		   //检查source不能是target的父节点，或者是父节的父节点.......
              ...
              //**********************************************************************
              if(ns)
                  var cperror = targetTree.contentWindow.checkParnetIdError(sourceNodeId,targetNodeId);
              else
                  var cperror = targetTree.checkParnetIdError(sourceNodeId,targetNodeId);
              //**********************************************************************
              if(cperror == true){
                 //激活"更改父类"按扭
...
location.href="<gdp:context/>/privilege/resourceOrderAction.do?action=exchengId&id1="+sourceNodeId+"&id2="+targetNodeId;
}

function moveid(){
	
location.href="<gdp:context/>/privilege/resourceOrderAction.do?action=moveId&id1="+sourceNodeId+"&id2="+targetNodeId;
}

function exchangeparentid(){
	//alert(sourceNodeId+":"+targetNodeId);
	//return;
location.href="<gdp:context/>/privilege/resourceOrderAction.do?action=exchengParentId&id1="+sourceNodeId+"&id2="+targetNodeId;
}
</script>
</html>
