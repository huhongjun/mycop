    ...
    private static boolean checkExpression(String strExpression) {
        ...
        for (int idx = 0; idx < strExpression.length(); idx++) {
            ...
            if (c == ')') {
                ...
                if (! (isOperator(c) || (iChar >= 48 && iChar <= 57) ||
                       (c == '.'))) {
    ...
    private static String toResult(char operator, String sOpertand1,
        ...
        switch (operator) {
            ...
            case '/': //除
                ...
                break;
            case '%': //求余操作
                double dblOpertand1, dblOpertand2;
        ...
        try {
            while (true) {
                ...
                if (isOperator(strExpression.charAt(pos)) && bNegative == false) {
                    ...
                    strValue = "";
                    if (!operator.isEmpty()) {
                        if (priority(strExpression.charAt(pos)) <=
                            priority( ( (String) operator.lastElement()).charAt(
                            0))) {
                    ...
                    //递归调用本方法,计算括号内的表达式
                    strValue = procSum(strExpression.substring(pos + 1,
                        pos + rightPos + 1));
                    pos += rightPos + 2;
