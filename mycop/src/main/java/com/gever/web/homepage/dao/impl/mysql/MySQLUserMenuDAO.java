 ...
 * @author Hu.Walker
 */
public class MySQLUserMenuDAO extends DefaultUserMenuDao {
    /**
     * 当用户重新更改过权限时
    ...
    public boolean resetUserMenus(String userID, Collection resourceList)
        ...
        try {
            ...
            sbSql.append(userID).append("' and (nodeid  in (select cast(id as char) from T_RESOURCE) ");
            sbSql.append(
                " and parentid in (select cast(parent_id as char) from T_RESOURCE))");
            ...
            sbSql.append("    CAST(A.id AS CHAR),A.NAME, ");
            sbSql.append("     CAST(A.parent_id AS CHAR),A.link as link,");
            ...
            //huhj:sbSql.append(" where CAST(A.id as char) not in (");
            //huhj:sbSql.append("select nodeid from T_USER_MENU where empid='");
            //huhj:sbSql.append(userID).append("')");
            ...
            sbSql.append("FROM T_USER_MENU where empid='").append(userID)
                 .append("'");
            sbSql.append(
                " and nodeid in (select CAST(id as char) from T_RESOURCE)");
            ...
            while (aryResult.hasNext()) {
                ...
                for (int row = 0; row < aryMenus.size(); row++) {
                    ...
                    if (menuVo.getNodeid().trim().equals(String.valueOf(
                                    vo.getId()))) {
            ...
            log.console(this.getClass(),"resetUserMenu.SQL",sbSql.toString());
            helper.execUpdate(sbSql.toString());
    ...
    //由于此方法引起用户登录或权限改变时改变用户自定义菜单，因此改写此方法，具体实现见上一方法
    //    public boolean resetUserMenus(String userID, Collection resourceList)
    //        throws DAOException {
    //        boolean bRet = true;
    //        //log.showLog("------------start---");
    ...
    //            sbSql.setLength(0);
    //            sbSql.append(
    //                " SELECT COUNT(*) AS CNT FROM T_USER_MENU where EMPid='");
    //            sbSql.append(userID).append("'");
    //
