...
public class PersonWeekTargetAction
    extends BaseAction {
  PersonWeekTargetDao personweektargetDao; //定义接口
  public PersonWeekTargetAction() {
  }
  ...
  protected void initData(GoaActionConfig cfg) throws DefaultException,
      Exception {
    PersonWeekTargetForm myForm = (PersonWeekTargetForm) cfg.getBaseForm(); //得到form变量
    personweektargetDao = PersonWeekTargetFactory.getInstance().
        createPersonWeekTarget(super.dbData); //得到相对应dao的实例
    cfg.setBaseDao( (BaseDAO) personweektargetDao); //cfg--Goa Action获取类--设置超类中的Dao
//容错处理,防止vo对象为null
  ...
  protected String toList(GoaActionConfig cfg) throws DefaultException,
    ...
    String user_dev = (String) session.getAttribute(Constant.USER_FILTER);
    ...
    if ("true".equals(queryFlag)) {
      String searchYear = ( (PersonWeekTargetForm) cfg.getBaseForm()).
          getSearchYear();
      String searchMonth = ( (PersonWeekTargetForm) cfg.getBaseForm()).
          getSearchMonth();
      String searchWeek = ( (PersonWeekTargetForm) cfg.getBaseForm()).
          getSearchWeek();
      String searchUserName = ( (PersonWeekTargetForm) cfg.getBaseForm()).
          getSearchUserName();
      searchUserName = StringUtils.replaceText(searchUserName);
      //String searchUserCode = findUserCodeByUserName(searchUserName);//通过用户名找用户代码以便查询
      ...
      if (!StringUtils.isNull(searchUserName)) { //如果输入了查询用户的话添加该条件
        ...
        tempSearchUserCodes = systemMngUtil.getUserIdsByUserName(searchUserName);
      ...
      sbSqlWhere.append(" and TARGET_TYPE=" +
                        Integer.parseInt(ConstantSet.PersonWeekTargetType) +
                        " ");
      if (!isOracle()) {
        ...
        sbSqlWhere.append(
            " and CURRENT_MONTH IS NOT NULL and CURRENT_WORK IS NOT NULL");
      }
      sbSqlWhere.append(" and DEPT_CODE like '%" + curListType + "," +
                        "%' ");
      sbSqlWhere.append(" order by  CURRENT_MONTH DESC,CURRENT_WORK DESC ");
    }
    else {
      if (StringUtils.isNull(leftDeptType) == false) {
        if(StringUtils.isNull(tempDeptId)){//如果没有部门则不显示
          sbSqlWhere.append(" and 1<>1");
        }
        sbSqlWhere.append(" and ( ").append(user_dev).append("<b.level"); //只显示用户级层小于等于当前用户的
        sbSqlWhere.append(" or user_code= ").append(curUserCode).append(")");
  ...
  protected String toEdit(GoaActionConfig cfg) throws DefaultException,
    ...
    if (!this.MODIFY_ACTION.equals(actionType)) { //如果是到新增页面-需要设置年月周
      ...
      //curWeek = DateTimeUtils.getWeekNumOfThisMonth();
    ...
    vo.setTarget_type(ConstantSet.PersonWeekTargetType);
    ...
    creatorFlag = ConstantSet.judgeCreatorTag(curUseCode, user_code, auditor,
                                              audit_flag);
    System.out.println("The creatorFlag is----" + creatorFlag);
  ...
  protected String doInsert(GoaActionConfig cfg, boolean isBack) throws
    ...
    PersonWeekTargetForm personweektargetForm = (PersonWeekTargetForm) cfg.
        getBaseForm();
    PersonTargetVO vo = (PersonTargetVO) personweektargetForm.getVo();
    vo.setTarget_content(Codes.decode(personweektargetForm.getCellcontent().
                                      toCharArray()));
    ...
    //设置审核人
    String auditor = personweektargetForm.getAuditorid();
    ...
    String auditorContent = "您好!有您要审核的人周目标!";
    String receiver1 = vo.getAuditor();
    String webUrl = "/dailyoffice/worklog/framepersonweektarget.jsp?fid="+vo.getSerial_num();
  ...
  protected String doUpdate(GoaActionConfig cfg, boolean isBack) throws
    ...
    vo.setTarget_content(Codes.decode(personweektargetForm.getCellcontent().
                                      toCharArray()));
    //取消审核
    if ("1".equals(personweektargetForm.getIsCancel())) {
      vo.setAudit_flag("0");
      vo.setAudit_date("");
      vo.setAudit_opinion("");
    }
    String auditorId = "";
    auditorId = personweektargetForm.getAuditorid();
...
//创建临时文件以便查询
  private void makeCellField(GoaActionConfig cfg) throws DefaultException,
    ...
    personweektargetForm.setCellname(strFilename);
    byte[] bcell = vo.getTarget_content();
    System.out.println("Content is:--" + bcell.toString());
  ...
  public String doTreeData(GoaActionConfig cfg) throws DefaultException,
    ...
    if(!StringUtils.isNull(user_dev)){
      if (iuser_dev<=3) {
        cfg.getRequest().setAttribute("treeData",personweektargetDao.getTreeData(nodeid));
      }else{
        cfg.getRequest().setAttribute("treeData",
                                      personweektargetDao.getTreeData(nodeid, curDeptId));
      }
    }
    return TREE_PAGE;
  }
