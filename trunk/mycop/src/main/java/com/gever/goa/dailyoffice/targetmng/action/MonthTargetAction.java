...
public class MonthTargetAction
    extends BaseAction {
    MonthTargetDao monthtargetDao = null; //定义接口
    OrganizationUtil util = new OrganizationUtil();
    public MonthTargetAction() {
    }
    ...
    protected void initData(GoaActionConfig cfg) throws DefaultException,
        Exception {
        MonthTargetForm myForm = (MonthTargetForm) cfg.getBaseForm(); //得到form变量
        monthtargetDao = MonthTargetFactory.getInstance().createMonthTarget(super.
            dbData); //得到相对应dao的实例
        cfg.setBaseDao( (BaseDAO) monthtargetDao); //cfg--Goa Action获取类--设置超类中的Dao
        //容错处理,防止vo对象为null
    ...
    protected String toList(GoaActionConfig cfg) throws DefaultException,
        ...
        if ("true".equals(queryFlag)) {
            String searchYear = ( (MonthTargetForm) cfg.getBaseForm()).getSearchYear();
            String searchMonth = ( (MonthTargetForm) cfg.getBaseForm()).getSearchMonth();
           ...
           sbSqlWhere.append(" and TARGET_TYPE='" +ConstantSet.MonthTargetType + "' ");
    ...
    protected String toEdit(GoaActionConfig cfg) throws DefaultException,
        ...
        vo.setTarget_type(ConstantSet.MonthTargetType);
    ...
    protected String doInsert(GoaActionConfig cfg, boolean isBack) throws
        ...
        HttpSession session = cfg.getSession();
        MonthTargetForm myForm = (MonthTargetForm) cfg.getBaseForm();
        TargetVO vo = (TargetVO) myForm.getVo();
        vo.setTarget_content(Codes.decode(myForm.getCellcontent().
                                          toCharArray()));
        /*
                 String curDept = (String) session.getAttribute("curDept");
                 //这里得到的是空值，说明在页面跳转的过程中将session给清空了？？？？？？
         if ("".equals(curDept) || curDept == null) { //如果从session中获取的当前部门ID为空则从formbean中获取
            curDept = myForm.getDeptnodeid();
                 }
                 vo.setDept_code(curDept);*/
        //用来判断用户填写的资料是否已经存在于数据库中了
        String current_year = vo.getCurrent_year();
        System.out.println("---------The current_year is----------- :" +
                           current_year);
        String current_month = vo.getCurrent_month();
        System.out.println("0--------The current_month is:-----------:" +
                           current_month);
    ...
    private void sendMsg(TargetVO vo) throws Exception {
        ...
        String auditorContent = "您好!目标管理中有您需要审核的月目标!";
        String checkerContent = "您好!目标管理中有您需要审批的月目标!";
        String receiver1 = vo.getAuditor();
        String receiver2 = vo.getChecker();
        String webUrl = "/dailyoffice/targetmng/framemonthtarget.jsp?fid=" + vo.getSerial_num();
        messageDao.sendMessage(userCode, sendTime, auditorContent, receiver1, webUrl, "转向月目标审核");
        messageDao.sendMessage(userCode, sendTime, checkerContent, receiver2, webUrl, "转向月目标审批");
    }
    ...
    protected String doUpdate(GoaActionConfig cfg, boolean isBack) throws
        ...
        if ("1".equals(isCancel)) {
            //vo.setTarget_content(Codes.decode(vo.getTarget_content().toString().toCharArray()));
            vo.setAudit_flag("0");
            vo.setAudit_date("");
            vo.setAudit_opinion("");
        }
    ...
    private void makeCellField(GoaActionConfig cfg) throws DefaultException,
        ...
        MonthTargetForm monthtargetForm = (MonthTargetForm) cfg.getBaseForm();
        TargetVO vo = (TargetVO) monthtargetForm.getVo();
        ...
        System.out.println("The templete name is ***:" + strFilename);
        monthtargetForm.setCellname(strFilename);
        byte[] bcell = vo.getTarget_content();
        // System.out.println("The vo of content is:***"+vo.getTarget_content().toString());
    ...
    public String doTreeData(GoaActionConfig cfg) throws DefaultException,
        ...
        if (!StringUtils.isNull(user_dev)) {
            if (iuser_dev <= 3) {
                cfg.getRequest().setAttribute("treeData", monthtargetDao.getTreeData(nodeid));
            } else {
                cfg.getRequest().setAttribute("treeData",
                                              monthtargetDao.getTreeData(nodeid, curDeptId));
            }
        }
        return TREE_PAGE;
    }
