...
public class WeekTargetAction extends BaseAction {
    WeekTargetDao weektargetDao; //定义接口
    OrganizationUtil util=new OrganizationUtil();
    public WeekTargetAction() {
    }
    ...
    protected void initData(GoaActionConfig cfg) throws DefaultException,
        Exception {
        WeekTargetForm myForm = (WeekTargetForm) cfg.getBaseForm(); //得到form变量
        weektargetDao = WeekTargetFactory.getInstance().createWeekTarget(super.
            dbData); //得到相对应dao的实例
        cfg.setBaseDao( (BaseDAO) weektargetDao); //cfg--Goa Action获取类--设置超类中的Dao
//容错处理,防止vo对象为null
    ...
    protected String toList(GoaActionConfig cfg) throws DefaultException,
        ...
        if ("true".equals(queryFlag)) {
            String searchYear = ( (WeekTargetForm) cfg.getBaseForm()).getSearchYear();
            String searchMonth = ( (WeekTargetForm) cfg.getBaseForm()).getSearchMonth();
            String searchWeek = ( (WeekTargetForm) cfg.getBaseForm()).getSearchWeek();
            ...
            sbSqlWhere.append(" and TARGET_TYPE='" + ConstantSet.WeekTargetType + "' ");
            sbSqlWhere.append(" and ").append(super.getIsNotNull("CURRENT_MONTH"));
            sbSqlWhere.append(" and ").append(super.getIsNotNull("CURRENT_WORK"));
            if(isTreeDept){
    ...
    protected String toEdit(GoaActionConfig cfg) throws DefaultException,
        ...
        if (StringUtils.isNull(myForm.getReportTemplateName())) { //如果报表模板还没有设置
            ...
            IsStandardModelVO ismVO = (IsStandardModelVO) isStandardModelDao.getTemplate(ConstantSet.WeekTargetTemplate);
        ...
        String deptsCodeOfCurUser =util.getDepartmentIDsByUser(Integer.parseInt(curUser));
        if(!StringUtils.isNull(deptsCodeOfCurUser)){
          deptsCodeOfCurUser = deptsCodeOfCurUser.substring(0, deptsCodeOfCurUser.indexOf(","));
        }
        String deptsNameOfCurUser = util.getDepartmentNamesByUser(Integer.parseInt(curUser));//得到部门ID所对应的部门名称
        String curDate = DateTimeUtils.getCurrentDate(); //取得当前日期--对应创建日期
        String curYear = "";
        String curMonth = "";
        String curWeek = "";
        System.out.println("The actionType is:***"+actionType);
        if (!this.MODIFY_ACTION.equals(actionType)) { //如果是到新增页面-需要设置年月周
            ...
            curWeek = DateTimeUtils.getCurrentWeek();
        ...
        vo.setTarget_type(ConstantSet.WeekTargetType);
        ...
        isReadOnlyTemplateFlag = ConstantSet.judgeReadOnlyTemplateTag(curUser, user_code, check_flag, checker,
            audit_flag, auditor);
        //用来判断是否显示审核或审批信息
        String flag = SumUtils.nullToString(cfg.getRequest().getParameter(
            "flag"));
        System.out.println("The flag is :--"+flag);
        auditFlag = ConstantSet.judgeAuditTag(curUser, auditor, audit_flag, check_flag);
        checkFlag = ConstantSet.judgeCheckTag(curUser, checker, audit_flag, check_flag);
        ...
        if (auditFlag == 1&&!flag.equals("modify")) { //当前人是审核人时
            ...
            vo.setAudit_date(curDate); //将审核日期设置为当前的日期
        } else if (auditFlag == 0) {
        ...
        if (checkFlag == 1&&!flag.equals("modify")) { //当前人是审批人时
            ...
            vo.setCheck_date(curDate); //将审批日期设置为当前的日期
        } else if (checkFlag == 0) {
    ...
    protected String doInsert(GoaActionConfig cfg, boolean isBack) throws
        ...
        HttpSession session = cfg.getSession();
        WeekTargetForm myForm = (WeekTargetForm) cfg.getBaseForm();
        ...
        String isExistFlag = constantSet.queryTargetIsExist(current_year, current_month, current_week, target_type,
            dept_code, user_code);
        ...
        String retStr = "";
        try {
           // retStr = super.doInsert(cfg, isBack);
    ...
    private void sendMsg(TargetVO vo) throws Exception {
      MessageDao messageDao = MessageFactory.getInstance().
       ...
       String auditorContent = "您好!目标管理中有您需要审核的周目标!";
       String checkerContent = "您好!目标管理中有您需要审批的周目标!";
       String receiver1 = vo.getAuditor();
       String receiver2 = vo.getChecker();
       String webUrl = "/dailyoffice/targetmng/frameweektarget.jsp?fid="+vo.getSerial_num();
       messageDao.sendMessage(userCode, sendTime, auditorContent, receiver1, webUrl,"转向周目标审核");
       messageDao.sendMessage(userCode, sendTime, checkerContent, receiver2, webUrl,"转向周目标审批");
    }
    ...
    protected String toView(GoaActionConfig cfg) throws DefaultException,
        ...
        System.out.println("The auditor is:***"+auditorid);
        System.out.println("The checkerid is:****"+checkerid);
        ...
        if (!"".equals(auditorid) && auditorid != null) {
            ...
            if (util.checkUser(iAuditorid)) {
                ...
                //设置数据库中获取的审核人显示到页面
                 System.out.println("The auditName is:***"+auditorName);
            }else{
                myForm.setAuditorname("");
            }
        }else{
        ...
        if (!"".equals(checkerid) && checkerid != null) {
            ...
            if (util.checkUser(iCheckerid)) {
                ...
                String checkerName = userChecker.getName();
                myForm.setCheckername(checkerName); //设置数据库中获取的审批人显示到页面
                System.out.println("The checkerName is:***"+checkerName);
            }else{
                myForm.setCheckername("");
            }
        }else{
        ...
        creatorFlag = ConstantSet.judgeEditTagForViewOfNew(curUser, usercode, checkflag, checkerid, auditflag,
            auditorid);
        System.out.println("The creator flag is:"+creatorFlag);
        if(creatorFlag==1){
        ...
        //判断是否显示修改按钮
        int modify_flag=ConstantSet.judgeModify(curUser,usercode,auditorid,checkerid,auditflag,checkflag);
        if(modify_flag==1){
        ...
        System.out.println("The modify_falg is***:"+modify_flag);
                //判断是否显示取消审核和取消审批
            int isCancel = ConstantSet.judgeIsCancel(curUser, auditorid,
...
//创建临时文件以便查询
    private void makeCellField(GoaActionConfig cfg) throws DefaultException,
        ...
        WeekTargetForm weektargetForm = (WeekTargetForm) cfg.getBaseForm();
        TargetVO vo = (TargetVO) weektargetForm.getVo();
        ...
        weektargetForm.setCellname(strFilename);
    ...
    if(!StringUtils.isNull(user_dev)){
      if (iuser_dev<=3) {
        cfg.getRequest().setAttribute("treeData",weektargetDao.getTreeData(nodeid));
      }else{
        cfg.getRequest().setAttribute("treeData",
                                      weektargetDao.getTreeData(nodeid, curDeptId));
      }
    }
    return TREE_PAGE;
  }
