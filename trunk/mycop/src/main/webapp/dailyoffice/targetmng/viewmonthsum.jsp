...
<html:form action="/dailyoffice/targetmng/viewmonthsum" method="post">
...
<TR>
	<TD colspan="8" align="center" class="TableTitleText"><bean:write  name="MonthSumForm" property="vo.current_year" filter="true"/>年<bean:write  name="MonthSumForm" property="vo.current_month" filter="true"/>月总结</TD>
</TR>
...
<TABLE id="idTBBottom" style="POSITION:absolute; left: 5px; top: 199px;" cellpadding='0' cellspacing='0' width="100%">
	...
	<TD width="30%">
	  审　　核：<bean:write  name="MonthSumForm" property="auditorname" filter="true"/>	  
	</TD>
	<TD width="30%">
	  审　　批：<bean:write  name="MonthSumForm" property="checkername" filter="true"/>	  
	</TD>
     </TR>
	 <TR class="listcellrow" align="left">
		<TD width="20%">
     ...
     <tr>
		...
		<logic:equal name="MonthSumForm" property="isCancel" value = "1">
		 <goa:button operation="MZMB.OPT" property="cancelAudit" value="取消审核" styleClass="button"  onclick="doCancel()"/>	
		 </logic:equal>
		   <logic:equal name="MonthSumForm" property="isCancel" value = "2">
		 <goa:button operation="MZMB.OPT" property="cancelCheck" value="取消审批" styleClass="button"  onclick="doCancel()"/>	
		 </logic:equal>
         <logic:equal name="MonthSumForm" property="modifyFlag" value = "1">
         <% String temp="toUrl('"+context+"/dailyoffice/targetmng/editmonthsum.do?actionType=modify&flag=modify',false)";%>
         <goa:button operation="YDZJ.OPT" property="modify" value="修改" styleClass="button"  onclick="<%=temp%>"/>	  
         </logic:equal>
		 <logic:equal name="MonthSumForm" property="creatorFlag" value = "2">
		       <% String temp1="toUrl('"+context+"/dailyoffice/targetmng/editmonthsum.do?actionType=modify',false)";%>
         <goa:button operation="YDZJ.OPT" property="audit" value="审核" styleClass="button"  onclick="<%=temp1%>"/>	  
         </logic:equal>
		 <logic:equal name="MonthSumForm" property="creatorFlag" value = "3">
		      <% String temp2="toUrl('"+context+"/dailyoffice/targetmng/editmonthsum.do?actionType=modify',false)";%>
         <goa:button operation="YDZJ.OPT" property="check" value="审批" styleClass="button"  onclick="<%=temp2%>"/>	  
         </logic:equal>
		 <goa:button operation="YDZJ.VIEW" property="exit" value="返回" styleClass="button" onclick="doAction('goUrl(index)')"/>
		 <goa:button operation="YDZJ.VIEW" property="exit" value="打印预览" styleClass="button" onclick="mnuFilePrintPreview_click()"/>
		 <goa:button operation="YDZJ.VIEW" property="exit" value="导出到Excel文件" styleClass="button" onclick="mnuFileExportExcel_click()"/>
        </td>
        </td>
     </tr>
</TABLE>
