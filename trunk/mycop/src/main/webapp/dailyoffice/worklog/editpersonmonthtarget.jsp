...
function window.onload() {
	...
	<logic:equal name="PersonMonthTargetForm" property="actionType" value = "add">
	CellWeb1.OpenFile(getUrl() + "<bean:write  name="PersonMonthTargetForm" property="reportTemplateName" filter="true"/>","");
	</logic:equal>
	<logic:equal name="PersonMonthTargetForm" property="actionType" value = "modify">
	CellWeb1.OpenFile( getUrl() + "<bean:write  name="PersonMonthTargetForm" property="cellname" filter="true"/>","");
	</logic:equal>
	...
	<logic:equal name="PersonMonthTargetForm" property="creatorFlag" value = "0">
...
function check(isExit){//校验页面--通过后提交
	var tform = document.forms(0);	
	if(tform.elements("vo.current_month").value==""){
		alert("请填写月！");
		tform.elements("vo.current_month").focus();
		return false;
	}		
	if(tform.elements("vo.current_month").value>12 || tform.elements("vo.current_month").value<1){
...
<html:form action="/dailyoffice/worklog/editpersonmonthtarget" method="post">
<TABLE id="idTRTop" cellpadding='0' cellspacing='0' width="100%">
  <html:hidden property="cellcontent"/>
  <html:hidden name="PersonMonthTargetForm" property="actionType"/>
  <html:hidden name="PersonMonthTargetForm" property="auditorid"/>   
  <html:hidden name="PersonMonthTargetForm" property="vo.user_code"/>
<TR>
    <logic:equal name="PersonMonthTargetForm" property="actionType" value = "add">
	<TD colspan="10" align="center" class="TableTitleText">个人月度汇报</TD>
	</logic:equal>
	<logic:equal name="PersonMonthTargetForm" property="actionType" value = "modify">
	<TD colspan="10" align="center" class="TableTitleText">个人月度汇报</TD>
	</logic:equal>
...
<TR class="listcellrow">
	...
	<TD width="25%">部    门：<bean:write  name="PersonMonthTargetForm" property="deptname" filter="true"/></TD>	
	<TD width="25%">岗    位：<bean:write  name="PersonMonthTargetForm" property="postname" filter="true"/></TD>	
	<TD width="25%">姓    名：<bean:write  name="PersonMonthTargetForm" property="username" filter="true"/></TD>	
	<TD width="20%" align="left">
	<html:text styleClass="input0" readonly="true" property="vo.current_year" size="4"  maxlength="4"/>
	年
	<html:text styleClass="input0" property="vo.current_month" onlytype="int" size="2"  maxlength="2"/>
	月
	</TD>
</TR>
</TABLE>
...
<TABLE id="idTBBottom" style="POSITION:absolute; left: 5px; top: 199px;" cellpadding='0' cellspacing='0' width="100%">
	...
	<TD colspan="10" align="left">&nbsp;</TD>	
	 ...
	 <logic:equal name="PersonMonthTargetForm" property="auditFlag" value = "1">
	...
	<TD width="30%">编制：<bean:write  name="PersonMonthTargetForm" property="username" filter="true"/></TD>
	<TD width="30%" align="left">编制日期：<bean:write  name="PersonMonthTargetForm" property="vo.create_date" filter="true"/></TD>	
	<TD width="40%" >  
		  审　　核：
          <logic:equal name="PersonMonthTargetForm" property="creatorFlag" value = "1">
		  <html:text property="auditorname" size="10" readonly="true"  styleClass="input2"/>	
		  </logic:equal>
		  <logic:equal name="PersonMonthTargetForm" property="creatorFlag" value = "0">
		  <html:hidden  name="PersonMonthTargetForm" property="auditorname" />
		  <bean:write  name="PersonMonthTargetForm" property="auditorname" filter="true"/>
          </logic:equal>
		  &nbsp;
		  <logic:equal name="PersonMonthTargetForm" property="creatorFlag" value = "1">
     ...
     <tr>
       ...
       <goa:button operation="LYMB.OPT" property="save" value="保存" styleClass="button"  onclick="check(false)"/>
	  <goa:button operation="LYMB.OPT" property="saveAndExit" value="保存并返回" styleClass="button"  onclick="check(true)"/>	  	  
	  <goa:button operation="LYMB.OPT" property="reset" value="取消" styleClass="button"  onclick="pageUndo()"/>	  
      <goa:button operation="LYMB.VIEW" property="exit" value="返回" styleClass="button" onclick="doAction('goUrl(index)')"/></td>
     </tr>
</TABLE>
</html:form>
</BODY>
