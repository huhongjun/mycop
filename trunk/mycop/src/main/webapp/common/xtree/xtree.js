...
function WebFXTreeAbstractNode(sText, sAction,sNodeid) {
	this.childNodes  = [];
	this.nodeid =sNodeid || webFXTreeConfig.defaultNodeid;
...
function WebFXTree(sText, sAction, sBehavior, sIcon, sOpenIcon,sNodeid) {
...
function WebFXTreeItem(sText, sAction, eParent, sIcon, sOpenIcon,sNodeid) {
	this.base = WebFXTreeAbstractNode;
	this.base(sText, sAction);
	/* Defaults to close */
	if (webFXTreeConfig.usePersistence) {
...
WebFXTreeItem.prototype.toString = function (nItem, nItemCount) {
	...
	var str = "<div id=\"" + this.id + "\" ondblclick=\"webFXTreeHandler.toggle(this);\" class=\"webfx-tree-item\" onkeydown=\"return webFXTreeHandler.keydown(this, event)\">" +
		...
		"<img id=\"" + this.id + "-icon\" class=\"webfx-tree-icon\" src=\"" + ((webFXTreeHandler.behavior == 'classic' && this.open)?this.openIcon:this.icon) + "\" onclick=\"webFXTreeHandler.select(this);\">" +
			"<input type=\"checkbox\" name=\""+ this.id+"-check"+"\" onClick=\"toAction("+this.nodeid+")\">"+	
		"<a href=\"" + this.action + "\" id=\"" + this.id + "-anchor\" onfocus=\"webFXTreeHandler.focus(this);\" onblur=\"webFXTreeHandler.blur(this);\"" +
		(this.target ? " target=\"" + this.target + "\"" : "") +
		">" + label + "</a></div>" +
		"<div id=\"" + this.id + "-cont\" class=\"webfx-tree-container\" style=\"display: " + ((this.open)?'block':'none') + ";\">";
