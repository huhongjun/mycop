           ...
           while (it.hasNext()){
             res=(Resource)it.next();
                if (res.getChildNum()>0)
                  sb.append("<tree nodeid =\""+res.getId()+"\" text=\""+res.getName()+"\" src=\""+context+"/privilege/resourceAction.do?action=getTreeChild&#38;resid="+res.getId()+"\" Script=\"selectItem('"+res.getId()+"')\" isFolder=\"true\"/>");
                else
                  sb.append("<tree  nodeid =\""+res.getId()+"\" text=\""+res.getName()+"\" src=\""+context+"/privilege/resourceAction.do?action=getTreeChild&#38;resid="+res.getId()+"\"  Script=\"selectItem('"+res.getId()+"')\"  isFolder=\"true\" />");
           }
