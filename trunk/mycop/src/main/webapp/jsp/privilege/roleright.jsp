...
class RecurResource{
    ...
    for (int i=0; i<resources.size(); i++){
      ...
      if (resourceID == resource.getParentID()){
        ...
        for (int j=0; j<oprList.size(); j++){
          ...
          output.append(" value=\""+operation.getResourceID()+"o"+operation.getId()+"\" type=\"checkbox\" cid=\"o"+resource.getId()+"\" id=\"r"+resource.getId()+"\" value=\"\">"+operation.getName()+"&nbsp;");
        }
        if (isHasChildren){
  ...
  public boolean isHasOperationRight(long operationID){
    ...
    for (int i=0; i<user_res_opt_collect.size(); i++){
      RolePermission permission = (RolePermission)user_res_opt_collect.get(i);
...
<table width="80%" align="center">
  <tr>
    <td class="TableTitleText">角色权限分配</td>
  </tr>
<tr><td>
<table class="f12">
<%
...
<input type="hidden" name="action" value="savaRolePerm" />
