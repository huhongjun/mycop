...
//--Modified by linhs 2004-3-5 [begin]--
...

{
	...
	var msg="";
//	alert("hahah");
	//-- Added by linhs [begin] --
	if (strAlert != "" && strAlert != null){
		msg = "--" + strAlert + "\n";
	}
	//-- Added end --
	//--Modified by linhs 2004-3-5 [ end]--
	...
	for(var i=0;i<frmObj.elements.length;i++){
		...
		// 2005-04-06 李炽明 修改－增加对disabled控件的过滤
		if(el.ValidateType!=null && !(el.disabled)){
...
// 验证字符串是否是时间 add by 颜燕 (2004-8-26)
function isTimeStr(str){
	if( isEmpty( str ) ) return true;
	if(checkExp( /^[0-2]?\d:[0-5]?\d:[0-5]?\d$/g, str )){
		var arr2=str.split(":");
		var hour=arr2[0];   //[0,23]
		var minute=arr2[1]; //[0,59]
		var second=arr2[2]; //[0,59]
		if(!((0<=hour)&&( hour <= 23 )&& ( 0 <= minute ) && ( minute <=59  )&& ( 0 <= second ) && ( second <= 59 )))
		return false;
	}
	else if(checkExp( /^[0-2]?\d:[0-5]?\d$/g, str ) ){
		var arr=str.split(":");
		var hour=arr[0];
		var minute=arr[1];
		if(!((0<=hour)&&( 23 >= hour )&&( 59 >= minute ) && (0 <= minute )))
...
// 验证是否日期加 时间且不能为空 add by 张碧娟 (2003-7-29)
function isTime3( strValue )
{
	// 日期格式必须是 2001-10-1 18:30 或者为空

	if( isEmpty( strValue ) ) return true;
	//if( !checkExp( /^\d{4}-[01]?\d-[0-3]?\d+\s+[0-2]?\d:[0-6]?\d:[0-6]?\d$/g, strValue ) ) return false;
	if( !checkExp( /^\d{4}-[01]?\d-[0-3]?\d+\s+[0-2]?\d:[0-6]?\d$/g,          strValue ) ) return false;
	// 或者 /^\d{4}-[1-12]-[1-31]\d$/
...
function ValidateEl( obj, strType)
{
	...
	switch( strType.toLowerCase() )
	{
		...
		case "time":    //日期加时间
			...
			break;
		case "time3":    //日期加时间
			result=isTime3(strValue);
			if (result) {obj.value = formatTime3(strValue)}
			break;
		case "time1":    //日期加时间
...
function setBoxInputType(){
	...
	if(event.shiftKey && flag!="timeonly"&& flag!="time3"&& flag!="time"&& flag!="time1") {
	  return false;
	}
...
function checkTextBoxInput(){
	...
	if(txtBox.onlytype){
		...
		|| txtBox.onlytype.toLowerCase()=="time1"|| txtBox.onlytype.toLowerCase()=="time3")
		{
		...
		if(txtBox.onlytype.toLowerCase()=="int" || txtBox.onlytype.toLowerCase()=="float") {
...
function selectDateNoTime() {
	var str=window.showModalDialog(context+"/jscript/getdateNoTime.html",null,"font-size:10px;dialogWidth:180px;dialogHeight:180px;scrollbars=no;status=no");
...
function selectDate() {
	var str=window.showModalDialog(context+"/jscript/getdate.html",null,	"font-size:10px;dialogWidth:180px;dialogHeight:180px;scrollbars=no;status=no");
	if (str == null || str == "" || str == "undefined")
		return "";
	else
		return str;
...
function setTextBoxDate()
{
	...
	if(this.onlytype.toLowerCase()=="date"){
		...
		this.onlytype.toLowerCase()=="time3"){
		bl=false;
	}
	if(bl){
		ret=selectDateNoTime();
		...
		if(ret!=null && ret!=""){
			if (this.onlytype.toLowerCase()=="time3"){
